...
def generate_content_safe(api_key, contents):
    """Gọi trực tiếp gemini-1.5-flash để tiết kiệm Token và tránh dính Rate Limit"""
...
# BƯỚC 2
st.markdown('<div class="step-header">🎯 BƯỚC 2: YÊU CẦU CẦN ĐẠT</div>', unsafe_allow_html=True)
...
integrations = st.multiselect(
    "Lựa chọn tích hợp:",
    ["Ứng dụng CNTT (Padlet, Kahoot, Geogebra, Azota...)", "Tích hợp AI trong dạy học (Gemini, ChatGPT, Canva...)", "Giáo dục STEM / STEAM", "Phát triển Tư duy phản biện & Tự học"],
...
if st.button("🚀 BẤM TẠO GIÁO ÁN WORD CHUẨN 5512", type="primary", use_container_width=True):
    if not api_key:
        ...
        try:
            ...
            III. TIẾN TRÌNH DẠY HỌC (Đầy đủ {duration} tiết):
            ...
            with st.spinner("⚡ Đang tạo Giáo án Word 5512..."):
