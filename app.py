...
input_mode = st.radio(
    ...
    ["📂 Đọc từ Tệp đính kèm (SGV / Cấu trúc chương trình PDF/Ảnh)", "🤖 Tra cứu tự động bằng AI Gemini", "✍️ Nhập thủ công"],
    horizontal=True
)
...
if "📂 Đọc từ Tệp" in input_mode:
        ...
        if st.button("⚡ Phân tích Tệp & Trích xuất Tên bài + Mục tiêu chuẩn SGV", type="primary"):
            with st.spinner("🔍 AI đang đọc chính xác nguyên văn SGV từ file..."):
                try:
                    ...
                    {
                        "chapter": "Tên Chương/Chủ đề (Nếu có)",
                        "lesson": "Tên bài học đầy đủ",
                        "duration": "Số tiết thực hiện (Số nguyên)",
                        "requirements": "Liệt kê chính xác, đầy đủ từng gạch đầu dòng các Yêu cầu cần đạt (Mục tiêu kiến thức, kỹ năng) nguyên văn theo SGV"
                    }
                    """
    ...
    if 'extracted_data' in st.session_state:
        ...
        chapter_title = st.text_input("Chương / Chủ đề:", value=ext.get('chapter', ''))
        lesson_title = st.text_input("Tên Bài dạy đầy đủ (Chuẩn SGV):", value=ext.get('lesson', ''))
        try:
    ...
    if 'ai_lessons' in st.session_state:
        ...
        lesson_title = st.text_input("Tên Bài dạy đầy đủ (Chuẩn SGV):", value=sel.get('lesson', ''))
        try:
    ...
    lesson_title = st.text_input("Tên Bài dạy đầy đủ (Chuẩn SGV):", value="")
    duration = st.number_input("Số tiết thực hiện:", value=3, min_value=1, max_value=20)
    requirements = st.text_area("Yêu cầu cần đạt chuẩn SGV:", value="", height=150)
...
def generate_doc(content_text):
    ...
    if chapter_title:
        ...
        r_chap = p_chap.add_run(chapter_title.upper())
        r_chap.bold = True
