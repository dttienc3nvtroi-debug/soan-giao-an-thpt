...
with col_left:
    ...
    if st.button("🔍 Tra cứu Danh sách Bài học chuẩn NXB Giáo Dục", use_container_width=True):
        if not clean_api_key:
            ...
            with st.spinner("⚡ Đang kết nối dữ liệu NXB Giáo Dục..."):
                try:
                    prompt_lookup = f"""
                    Bạn là Chuyên gia dữ liệu SGK Việt Nam. Hãy tra cứu chính xác Chương trình SGK môn {subject} {grade} - Bộ sách {book_series} phát hành trên cổng NXB Giáo dục Việt Nam (taphuan.nxbgd.vn).
                    Hãy trả về duy nhất mảng JSON chứa danh sách bài học:
                    [
                        {{
                            "chapter": "Tên Chương/Chủ đề chuẩn NXB",
                            "lesson": "Tên Bài học nguyên văn SGK",
                            "duration": 3
                        }}
                    ]
                    """
                    ...
                    if isinstance(data, list):
                        ...
                        st.success(f"✅ Đã tìm thấy {len(data)} bài học chuẩn NXB!")
    ...
    if 'ai_lessons' in st.session_state:
        ...
        selected_idx = st.selectbox("Chọn Bài học từ danh sách NXB:", range(len(options)), format_func=lambda i: options[i])
        ...
        lesson_title = st.text_input("Tên Bài dạy nguyên văn SGK (Nhập thủ công nếu chưa tra cứu):", value="")
...
if st.button("🚀 BẮT ĐẦU TẠO KHBD WORD CHUẨN 5512", type="primary", use_container_width=True):
    if not clean_api_key:
        ...
        try:
            ...
            - YÊU CẦU CẦN ĐẠT SGV / MỤC TIÊU BẮT BUỘC:
            {req_prompt}
            ...
            Trình bày rõ ràng, văn phong sư phạm chuẩn mực, chi tiết không tóm tắt.
            """
